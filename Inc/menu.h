 ...
 *  2026 FEB 18, v1.04
 *  	Added MG_ENC_MODE menu entry into MENU_GUN class
 */
...
class MENU_GUN : public MODE {
	...
	private:
		...
		bool		is_enc_speed	= false;				// Flag indicating the Encoder changes fan speed
		...
		const uint16_t	min_standby_C	= 120;				// Minimum standby temperature, Celsius
		enum { MG_FAST_CHILL = 0, MG_STBY_TO, MG_STANDBY_TEMP, MG_FAN_VOLTAGE, MG_ENC_MODE, MG_SAVE, MG_CALIBRATE, MG_BACK };
};
