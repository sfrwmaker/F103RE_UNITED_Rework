...
#define TFT_TIM		htim5
extern TIM_HandleTypeDef TFT_TIM;

class BRGT {
	public:
					BRGT(void)								{ }
		void		start(void)								{ HAL_TIM_PWM_Start(&TFT_TIM, TIM_CHANNEL_4);	}
		void		stop(void)								{ HAL_TIM_PWM_Stop(&TFT_TIM,  TIM_CHANNEL_4);	}
		uint8_t		get(void)								{ return TFT_TIM.Instance->CCR4;				}
		void		off(void)								{ TFT_TIM.Instance->CCR4 = 0;					}
		void		dim(uint8_t br)							{ TFT_TIM.Instance->CCR4 = br;					}
		void		on(void)								{ TFT_TIM.Instance->CCR4 = brightness;			}
...
class DSPL : public tft_ILI9341, public BRGT, public GRAPH, public NLS_MSG {
	...
	private:
		...
		const uint8_t*	big_dgt_font	= (uint8_t*)u8g2_font_kam28n;
		const uint8_t*	debug_font		= (uint8_t*)u8g_font_profont22r;
		const uint8_t*	def_font		= (uint8_t*)u8g_font_profont22r;
};
