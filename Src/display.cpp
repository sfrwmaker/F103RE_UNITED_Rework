...
bool BRGT::adjust(void) {
	...
	if (brightness > TFT_TIM.Instance->CCR4) {
		if (brightness - TFT_TIM.Instance->CCR4 > b_step) {
			TFT_TIM.Instance->CCR4 += b_step;
		} else {
			++TFT_TIM.Instance->CCR4;
		}
		return true;
	} else if (brightness < TFT_TIM.Instance->CCR4) {
		if (TFT_TIM.Instance->CCR4 - brightness > b_step) {
			TFT_TIM.Instance->CCR4 -= b_step;
		} else {
			--TFT_TIM.Instance->CCR4;
		}
...
void DSPL::clear(void) {
	// Do not turn-off backlight in the debug mode
#ifndef DEBUG_ON
...
void DSPL::drawFanPcnt(uint8_t p, bool modify) {
	...
	drawFilledRect(fan_pcnt_width, height()-h, width()-fan_pcnt_width, h, bg_color); // Clear the rest of the bottom line
}
...
void DSPL::drawTipList(TIP_ITEM list[], uint8_t list_len, uint8_t index, bool name_only) {
	...
	for (uint8_t i = 0; i <  list_len; ++i) {
		...
		if (!list[i].tip_name.isEmpty()) {					// The tip name is not empty
			...
			if (!name_only) {								// Draw calibrated check box
				margin += 10+16;
				checkBox(tip_entry, 10, 16, list[i].tip_name.isActivated());
			}
			strToBitmap(tip_entry, list[i].tip_name.tipName().c_str(), align_left, margin);
			if (!(list[i].tip_name.isCalibrated())) {		// The tip is not calibrated
...
void DSPL::directoryShow(const std::vector<std::string> &dir_list, uint16_t item, std::string status) {
	...
	uint8_t  h			= getMaxCharHeight() + 10;			// Extra space between menu lines
	uint16_t top		= h+12;
	uint8_t	 max_items 	= (height() - top) / h - 1;			// The number items in the list depend on display orientation
	...
	// Draw item info
	uint16_t y = height() - h - 10;
	DSPL::drawHLine(5, y, width() - 10, fg_color);
	y += 5;
	bm_menu.clear();
	strToBitmap(bm_menu, status.c_str(), align_right, 10, true);
...
void DSPL::debugShow(uint16_t data[11], bool iron_on, bool gun_on, bool iron_connected, bool gun_connected, bool gun_reed, bool type_jbc, bool tilt_stby, bool jbc_change, bool gtim_ok) {
	...
	for (uint8_t i = 0; i < 6; ++i) {									// Draw 6 lines of two columns
		// Draw left column
		uint16_t clr = fg_color;
		sprintf(buff, "%5d", data[2*i]);								// Left column value string
		strToBitmap(bm, item_name[2*i], align_left);					// Left column name
		strToBitmap(bm, buff, align_right);
		if (i == 0 && iron_on) clr = gd_color;							// Highlight the iron power
		drawBitmap(10, top+i*h, bm, bg_color, clr);
		// Draw right column
		clr = fg_color;
		bm.clear();
		...
		strToBitmap(bm, buff, align_right);
		if ((i == 0 && gun_on) || (i == 3 && !gtim_ok)) clr = gd_color;
		drawBitmap(width()/2+10, top+i*h, bm, bg_color, clr);
		bm.clear();
	}
	if (type_jbc) {
		sprintf(buff, "[%c.%c.%c]", tilt_stby?'s':' ', jbc_change?'c':' ', gun_reed?'r':' ');
	} else {
		sprintf(buff, "[%c.%c]", tilt_stby?'t':' ', gun_reed?'r':' ');
	}
	strToBitmap(bm, buff, align_center);
	drawBitmap(10, top+6*h, bm, bg_color, fg_color);
	bm.clear();
	sprintf(buff, "(%c-%c)", iron_connected?'i':' ', gun_connected?'g':' ');
	strToBitmap(bm, buff, align_center);
	drawBitmap(width()/2+10, top+6*h, bm, bg_color, fg_color);
}
