...
bool W25Q::loadPIDparams(PID_PARAMS* pid_params) {
	if (!mount())
		return false;
	W25Q::close();
	UINT br = 0;
	bool ret = false;
	PID_PARAMS tmp_record;
	if (FR_OK == f_open(&cfg_f, fn_pid, FA_READ | FA_OPEN_EXISTING)) {
		f_read(&cfg_f, (void *)&tmp_record, (UINT)sizeof(PID_PARAMS), &br);
		if (br ==  (UINT)sizeof(PID_PARAMS)) {
			if (PID_checkSum(&tmp_record, false)) {
				memcpy((void *)pid_params, (void *)&tmp_record, sizeof(PID_PARAMS));
				ret = true;
			}
		}
		f_close(&cfg_f);
...
bool W25Q::savePIDparams(PID_PARAMS* pid_params) {
	if (!mount())
		return false;
	W25Q::close();
	PID_checkSum(pid_params, true);
	bool ret = false;
	if (FR_OK == f_open(&cfg_f, fn_pid, FA_CREATE_ALWAYS | FA_WRITE)) {
		UINT written = 0;
		f_write(&cfg_f, (void *)pid_params, sizeof(RECORD), &written);
		ret = (written == sizeof(PID_PARAMS));
		f_close(&cfg_f);
	}
	umount();
	return ret;
...
int16_t W25Q::saveTipData(TIP* tip, bool keep) {
	...
	if (!new_entry) {										// Try to locate our tip in the file
		...
		while(true) {										// Looking for the tip
			f_read(&cfg_f, (void *)&tmp_tip, (UINT)sizeof(TIP), &br);
			if (br == (UINT)sizeof(TIP)) {
				if (tip->name.match(tmp_tip.name)) {
...
uint8_t W25Q::tipListReadNextItem(const char data[], uint8_t size) {
	if (!mount())											// Cannot mount W25Qxx flash
		return 0;
	...
	// Read next tip name from the list
	UINT	br = 0;
	f_read(&cfg_f, (void *)data, (UINT)size, &br);
	if (br > 0) {											// Searching for end of line character (\n or \r\n)
		for (uint8_t i = 0; i < br; ++i) {
			if (data[i] == '\n') {
				FSIZE_t	new_pos = cfg_f.fptr + i + 1 - br;
				f_lseek(&cfg_f, new_pos);					// Prepare to read next string
				if (i > 0 && data[i-1] == '\r')
					--i;
				return i;
			}
		...
		uint8_t d;
		UINT	br1= 0;
		while(true) {										// Read file until the end or end of line
			f_read(&cfg_f, (void *)&d, (UINT)size, &br1);
			if (br1 > 0) {
				if (d == '\n') break;
			} else {										// End of file
				break;
...
// Checks the CRC of the PID_PARAMS structure. Returns true if OK. Replace the CRC with the correct value if write is true
uint8_t W25Q::PID_checkSum(PID_PARAMS* pid_params, bool write) {
	uint16_t 	summ 		= 117;							// To avoid good check sum with all-zero, start with 117
	uint16_t    rec_summ 	= pid_params->crc;
	pid_params->crc				= 0;
	uint8_t*	d 			= (uint8_t*)pid_params;
	for (uint8_t i = 0; i < sizeof(PID_PARAMS); ++i) {
		summ <<= 1; summ += d[i];
	}
	bool res = (rec_summ == summ);
	if (write) pid_params->crc = summ;
	return res;
}
