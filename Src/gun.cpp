...
void HOTGUN::init(void) {
	...
	chill			= false;
	UNIT::init(fan_curr_avg_len, fan_off_value, fan_on_value, sw_avg_len, sw_off_value, sw_on_value);
    ...
    c_temp.reset();
    h_temp.length(hot_gun_len);								// The Hot Air Gun long-term average temperature
	h_temp.reset();
	d_power.length(ec);
	d_temp.length(ec);
...
void HOTGUN::fanFixed(uint16_t fan) {
	FAN_TIM.Instance->CCR2 = constrain(fan, 0, max_fan_speed);
}

void HOTGUN::fanControl(bool on) {
	if (mode == POWER_OFF) {
		FAN_TIM.Instance->CCR2	= (on)?fan_speed:0;
	}
}
			...
			if (fanSpeed() == 0) {							// No power supplied to the Fan
				...
				if (On) {
					if (isConnected()) {					// FAN && On && connected
						safetyRelay(true);
						uint16_t t = c_temp.read();			// Use current temperature to calculate required power value
		...
		case POWER_STBY:
			if (!On) {										// Start cooling the hot air gun
				...
				if (fast_cooling) {							// Set maximum fan speed in case of fast cooling
					FAN_TIM.Instance->CCR2 = max_cool_fan;
				}
			}
			break;
		case POWER_FIXED:
...
uint16_t HOTGUN::power(void) {
	...
	switch (mode) {
		...
		case POWER_FIXED:
			...
			break;
		case POWER_STBY:
			FAN_TIM.Instance->CCR2	= min_fan_speed;
			if (chill) {
				if (t < (low_temp - 2)) {
					chill = false;
					resetPID();
				...
				if (isConnected()) {
					...
					if (min_cool_tm && HAL_GetTick() >= min_cool_tm + cooling_to) {	// FAN && connected && min_cool_temp has not been changed during timeout
						if (!reach_cold_temp) {
							reach_cold_temp = true;
							fan_off_time = HAL_GetTick() + fan_extra_time;
							FAN_TIM.Instance->CCR2 = max_fan_speed;
						}
